var cursor = db.trips_9.find({pickup_geoid: {$exists: false}});
var n = 0;
while (cursor.hasNext()) {
	...
	//			print("dropoff_name: " + dropoff_name + ", dropoff_geoid" + dropoff_geoid);
				db.trips_9.update({"_id" : id}, {$set :  {"pickup_geoid" : pickup_geoid, "pickup_name" : pickup_name, "dropoff_geoid" : dropoff_geoid, "dropoff_name" : dropoff_name } } );
				n++;
				print(n);
			}
			else {
	//			print("out of town");
				db.trips_9.update({"_id" : id}, {$set :  {"pickup_geoid" : pickup_geoid, "pickup_name" : pickup_name, "dropoff_geoid" : 0, "dropoff_name" : "Out of town" } } );
			}
		}
	}
}