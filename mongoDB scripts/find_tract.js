var cursor = db.trips_1.find({pickup_geoid: {$exists: false}});
var n = 0;
while (cursor.hasNext()) {
	...
	if (Math.abs(trip_pickup[0]) > 0 && Math.abs(trip_pickup[0]) < 100 && Math.abs(trip_pickup[1]) > 0 && Math.abs(trip_pickup[1]) < 100 && Math.abs(trip_drop[0]) > 0 && Math.abs(trip_drop[0]) < 100 && Math.abs(trip_drop[1]) > 0 && Math.abs(trip_drop[1]) < 100) {
		var id = trip._id;
	...
	//			print("dropoff_name: " + dropoff_name + ", dropoff_geoid" + dropoff_geoid);
				db.trips_1.update({"_id" : id}, {$set :  {"pickup_geoid" : pickup_geoid, "pickup_name" : pickup_name, "dropoff_geoid" : dropoff_geoid, "dropoff_name" : dropoff_name } } );
				n++;
				print(n);
			}
			else {
	//			print("out of town");
				db.trips_1.update({"_id" : id}, {$set :  {"pickup_geoid" : pickup_geoid, "pickup_name" : pickup_name, "dropoff_geoid" : 0, "dropoff_name" : "Out of town" } } );
			}
		}
	}
}